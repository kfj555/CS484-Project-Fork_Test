...
statisticsRouter.get("/easy", (req: Request, res: Response) => {
  const { department, level, year } = req.query;
  // Normalize and validate level to avoid producing `undefined%`.
  const stringLevel = String(level).trim();
  let dbCourseLevel: string;
  if (stringLevel === "all") {
    dbCourseLevel = "%"; // SQL wildcard to match all course numbers
  } else {
    dbCourseLevel = `${stringLevel.at(0)}%`; // e.g., "100" -> "1%" to match "100", "101", etc.
  }
  const rows = db
    .prepare(
      `
      ...
      `
    ).all(department, dbCourseLevel, year) as {
      subj_cd: string;
      course_nbr: string;
      title: string;
      instructor: string;
